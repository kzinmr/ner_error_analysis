...
#            "finepostag": entry[2],
    ...
    def window_features(self, entry, w):
        features = {
            f"{w}:word": entry[0],
            f"{w}:postag": entry[1],
#            f"{w}:finepostag": entry[2],
            f"{w}:chunktag": entry[3],
...
class EntityExtractor:
    ...
    def evaluate(self, sentences: List[Sentence], fix_invalid_labels=True, decoder='greedy', k=5, debug=False, export_file=None, classification_report_file=None):
        """
        ...
        if fix_invalid_labels:
            ...
            if export_file is not None:
                with open(export_file, 'w') as f:
                    for s, ps, gs in zip(sentences, y_pred, y_test):
                        for ss, p, g in zip(s.entries, ps, gs):
                            f.write(f'{ss[0]}\t{p}\t{g}\n')
                        f.write('\n')
...
if __name__ == "__main__":
    ...
    ee.evaluate(sentences_test, export_file='outputs/crf_outputs/token_label_pred_crf_greedy_fix.txt', classification_report_file='outputs/crf_outputs/classification_report_crf_greedy_fix.txt')
    ee.evaluate(sentences_test, decoder='beam', export_file='outputs/crf_outputs/token_label_pred_crf_beam_fix.txt', classification_report_file='outputs/crf_outputs/classification_report_crf_beam_fix.txt')
